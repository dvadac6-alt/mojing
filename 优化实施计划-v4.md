...
| # | 优化项 | 状态 | 备注 |
|---|---|---|---|
| #2 | workspace 懒加载 | 调研完成，需专项 | 混合模式会负优化（多一次请求），必须整包瘦身 + 10 页面迁移。阻塞点：chapters/scenes/graph_edges 缺独立 list 端点；patchWorkspace 乐观更新模式被 6 页面依赖 |
| #9 | 工程化（测试/CI/上报） | 待做 | 前端 0 测试；GitHub Actions 自动构建发布；错误上报 |
| #10 | 暗色模式 / 空状态 / a11y | 待做 | 全局主题工作量较大；空状态引导、aria 标签 |
...
⏸️ #2 workspace 懒加载     ← 下一个专项（大重构，需补 3 个端点 + 10 页面迁移）
⏸️ #9 工程化（测试/CI）
⏸️ #10 剩余（暗色/空状态/a11y）
```
